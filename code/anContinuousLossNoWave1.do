// Main results with continuous crop loss measure without wave 1
// anContinuousLossNoWave1.do
...
eststo cfe_c_a:  xtreg contra_any croplostamount_pc   pass3 pass4 , fe cluster(id_hh)
estadd local fixed "\mco{Yes}" , replace
eststo cfe_c_tr: xtreg contra_trad croplostamount_pc   pass3 pass4 , fe cluster(id_hh)
estadd local fixed "\mco{Yes}" , replace
eststo cfe_c_mo: xtreg contra_mode croplostamount_pc   pass3 pass4 , fe cluster(id_hh)
estadd local fixed "\mco{Yes}" , replace
eststo cfe_pr:   xtreg pregnant croplostamount_pc   pass3 pass4 , fe cluster(id_hh)
estadd local fixed "\mco{Yes}" , replace
...
eststo cfe_int_c_a:  xtreg contra_any croplostamount_pc croplostXassets_w1  pass3 pass4 , fe cluster(id_hh)
estadd local fixed "\mco{Yes}" , replace
eststo cfe_int_c_tr: xtreg contra_trad croplostamount_pc croplostXassets_w1  pass3 pass4 , fe cluster(id_hh)
estadd local fixed "\mco{Yes}" , replace
eststo cfe_int_c_mo: xtreg contra_mode croplostamount_pc croplostXassets_w1  pass3 pass4 , fe cluster(id_hh)
estadd local fixed "\mco{Yes}" , replace
eststo cfe_int_pr:   xtreg pregnant croplostamount_pc croplostXassets_w1  pass3 pass4 , fe cluster(id_hh)
estadd local fixed "\mco{Yes}" , replace
...
file open table using `tables'/appendix_cont_croploss_wow1.tex, write replace
...
file write table "\label{tab:continuous_wow1}" _n
...
esttab cfe_pr cfe_br cfe_c_a cfe_c_tr cfe_c_mo using `tables'/appendix_cont_croploss_wow1.tex, append ///
    fragment ///
...
file open table using `tables'/appendix_cont_croploss_wow1.tex, write append
file write table "\addlinespace" _n 
file write table " & \multicolumn{5}{c}{With interaction with assets} \\" _n
file close table

esttab cfe_int_pr cfe_int_br cfe_int_c_a cfe_int_c_tr cfe_int_c_mo using `tables'/appendix_cont_croploss_wow1.tex, append ///
    fragment ///
	nogap nolines varwidth(55) label ///
    collabels(none) mlabels(none) eqlabels(none) ///
    nomtitles nonumber nodepvars noobs ///
