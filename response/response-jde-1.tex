...
appealing because it captures that we should expect shocks to have
a diminishing impact on fertility related outcome as the size of the
crop loss increase (which is also why the linear version is not 
very appealing; it assumes that a one TZS increase in crop loss has
...
The difference from the dummy result is that the pregnancy and
...
To show that the cut-off chosen does not have substantial effects on
the conclusions drawn we have added a table that shows the main results
for different cut-offs and the associated percentages of observations
that would be counted as having a crop loss.


[should we drop those between 0 and the crop loss cut-off?]
